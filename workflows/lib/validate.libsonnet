...

{
  validateReleaseBranch: job.new() + job.withSteps([
    ...
    + step.withRun(|||
      ...
      jq -e '.["${{ steps.extract_branch.outputs.branch }}"]' release.json && exit 0

      echo 'Invalid release branch: ${{ steps.extract_branch.outputs.branch }}. Make sure release.json includes an entry for ${{ steps.extract_branch.outputs.branch }}.'
      exit 1
    |||),
  ]),
